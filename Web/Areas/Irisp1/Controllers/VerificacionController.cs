...
namespace Web.Areas.Irisp1.Controllers
{
    ...
    public class VerificacionController : Controller
    {
        ...
        {
            var auditoriaTask = _iDbAdministracion.P_InsAuditoria(Convert.ToInt64(User.FindFirstValue("Identificacion")), "Ingreso Módulo", "Ingreso módulo IrisP1/Verificación", Convert.ToInt64(User.FindFirstValue("Identificacion")), HttpContext.Session.GetString("IpMaquina"));
            var aniosTask = _iDbVerificacionIris.F_GetAniosIrisP1();

            // Dominios únicos que necesita esta vista (varios combos "...Modal"/duplicados reutilizan el
            // mismo dominio, así que se consultan una sola vez cada uno).
            ...
            var tipoResultadoTask = _IDbDominios.F_GetDominiosIris(68);
            ...
            await Task.WhenAll(auditoriaTask, aniosTask, claseTask, modExpendioTask, clasiNarcotraficoTask, actividadTask,
                fuenteTask, entornoTask, zonaTask, delitoTask, tipoResultadoTask, tipoServicioTask, existenciaTask,
                estadosTask, especialidadTask);
            ...
            var existenciaData = (await existenciaTask).Data?.OrderBy(x => x.Descripcion).ToList();
            ...
            ViewBag.ddlExistenciaIrisP1 = new SelectList(existenciaData, "IdDominio", "Descripcion");
            ...
            ViewBag.ddlTipoExiste = new SelectList(existenciaData, "IdDominio", "Descripcion");
